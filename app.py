...
# CSS AGGIORNATO: GESTIONE STAMPA E GRAFICI FISSI
st.markdown(f"""
    <style>
        ...
        @media print {{
            ...
            .stButton, .stNumberInput, .stSelectbox, .stSlider, .stDateInput, .stTextInput, header, footer, #MainMenu {{
            ...
            /* GRAFICI A DIMENSIONE FISSA IN STAMPA (evita il taglio a destra) */
...
# 4. ANALISI COMPARATIVA MATERIALI E TEMPI
...
with col_intec:
    ...
    litri_m2 = spessore_calcolo # 1 mm su 1 m2 = 1 Litro
    litri_totali = superficie_m2 * litri_m2
    
    fusti_prodotto = litri_totali / 200.0 # 1 fusto = 200 Litri fissi
...
with col_chart1:
    fig_costi = go.Figure(go.Bar(x=['Sistema INTEC', 'Metodo Cliente'], y=[tot_generale_intec, tot_generale_cliente], marker_color=['#008F99', '#4A4A4A'], text=[f"{tot_generale_intec:,.2f} {valuta_simbolo}", f"{tot_generale_cliente:,.2f} {valuta_simbolo}"], textposition='auto', textfont=dict(color='white'), width=0.35))
...
with col_chart2:
    fig_ore = go.Figure(go.Bar(x=['Sistema INTEC', 'Metodo Cliente'], y=[ore_intec, ore_cliente], marker_color=['#008F99', '#4A4A4A'], text=[f"{ore_intec:.1f} h", f"{ore_cliente:.1f} h"], textposition='auto', textfont=dict(color='white'), width=0.35))
